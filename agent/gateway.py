...
class Gateway:
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        is_write = (
            tool in ("record_mastery", "flag_struggle", "notes.write")
            or "write" in tool
            or "record" in tool
        )
        if is_write:
            if not headers.get("if-match") or not headers.get("idempotency-key"):
                return self.deny(cmd, "write commands require valid if-match and idempotency-key headers")

        # ------------------------------------------------------------------
        # JOB 3 — AUTHORIZE: verify act, aud, and required scopes
        # ------------------------------------------------------------------
        ctx_act = getattr(self.ctx, "act", None)
        ctx_scopes = getattr(self.ctx, "scopes", frozenset())

        # Write operations must target the authorized learner
        if is_write and ctx_act:
            target = args.get("learner") or args.get("learner_id") or args.get("target")
            if target and target != ctx_act:
                return self.deny(cmd, f"unauthorized write target {target!r} != {ctx_act!r}")

        # A2A delegation act verification
        if cmd.kind == "a2a":
            token_act = headers.get("a2a-act") or args.get("act")
            if token_act and ctx_act and token_act != ctx_act:
                return self.deny(cmd, f"A2A delegation act mismatch {token_act!r} != {ctx_act!r}")
        ...
        if rewritten:
            ...
            call = self._to_tool_call(new_cmd)
            decision = Decision(verdict="rewrite", call=call, note="gateway rewritten for route/budget safety")
